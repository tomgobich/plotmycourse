...
router.post('/login', [LoginController, 'store']).as('login.store').use([middleware.guest(), middleware.turnstile()])
router.get('/register', [RegisterController, 'show']).as('register.show').use(middleware.guest())
router.post('/register', [RegisterController, 'store']).as('register.store').use([middleware.guest(), middleware.turnstile()])
router.post('/logout', [LogoutController, 'handle']).as('logout').use(middleware.auth())

router.group(() => {
  
  router.get('/', [ForgotPasswordsController, 'index']).as('index')
  router.post('/', [ForgotPasswordsController, 'send']).as('send').use(middleware.turnstile())
...
router.get('/invites/organization/:id/accept', [InvitesController, 'accept']).as('invites.organization.accept')
